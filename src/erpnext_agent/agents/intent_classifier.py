...
将当前请求分类为以下之一：
- data：用户想了解、查找、汇总某一个具体对象或单一期间的事实信息；
- action：用户想创建、修改或准备一份可审批的业务草稿；
- patrol：用户想做跨期比较（环比、同比、趋势）、业绩回顾、有范围的异常/风险巡检，
  或要求分析并给出下一步建议；
- clarify：请求没有足够业务意图，或只是寒暄、无关问题、无法安全判断。

边界：只问“是多少/有哪些”的具体单一事实查询交给 data；出现“环比、同比、趋势、业绩、
异常、预警、建议”，或需要多个期间的数字才能回答的比较，交给 patrol。
